...
import it.nicolabrogelli.imedici.adapters.AdapterItineraries;
...
public class ActivityItineraries extends AppCompatActivity
    ...
    private static final String TAG = ActivityItineraries .class.getSimpleName();
    ...
    private AdapterItineraries mAdapter;
    ...
    private ArrayList<String> mItineraryIds          = new ArrayList<>();
    private ArrayList<String> mItineraryNames        = new ArrayList<>();
    private ArrayList<String> mItineraryImages       = new ArrayList<>();
    private ArrayList<String> mItineraryDescription  = new ArrayList<>();
    private ArrayList<Double> mItineraryLatitudes    = new ArrayList<>();
    private ArrayList<Double> mItineraryLongitudes   = new ArrayList<>();
    private ArrayList<Float>  mItineraryDistances    = new ArrayList<>();
    private ArrayList<String> mItineraryMarkers      = new ArrayList<>();

    // To handle LocationDistance in String
    private ArrayList<String> mItineraryDistancesString = new ArrayList<>();
    ...
    AHBottomNavigationItem itemCategory;
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        itemCategory = new AHBottomNavigationItem("Itinerari", R.mipmap.ic_list_white_24dp, Color.parseColor("#FF009788"));

        setListView(); // Set listView view
        ...
        bottomNavigation.setOnTabSelectedListener(new AHBottomNavigation.OnTabSelectedListener() {
            ...
            public void onTabSelected(int position, boolean wasSelected) {
                ...
                switch (position) {
                    ...
                    case 1:
                        i = new Intent(ActivityItineraries.this, ActivityHome.class);
                        startActivity(i);
                    ...
                    case 2:
                        i = new Intent(ActivityItineraries.this, ActivityCharacters.class);
                        startActivity(i);
                        overridePendingTransition(R.anim.open_next, R.anim.close_main);
                        break;
        ...
        getSupportActionBar().setTitle("Itinerari Locazee");
        ...
        // Set adapter object
        mAdapter = new AdapterItineraries(this);
        ...
        mAdapter.setOnTapListener(new OnTapListener() {
            ...
            public void onTapView(int position) {
                ...
                Intent detailIntent = new Intent(getApplicationContext(), ActivityItineraryDetailScrolling.class);
                detailIntent.putExtra(Utils.ARG_LOCATION_ID, mItineraryIds.get(position));
                detailIntent.putExtra(Utils.ARG_ACTIVITY, Utils.TAG_ACTIVITY_ITINERARIES);
...
*/
    ...
    public void onInfoWindowClick(Marker marker) {
        ...
        Intent detailIntent = new Intent(this, ActivityItineraryDetailScrolling.class);
        ...
        switch (status.getStatusCode()) {
            ...
            case LocationSettingsStatusCodes.RESOLUTION_REQUIRED:
                ...
                try {
                    ...
                    // in onActivityResult().
                    status.startResolutionForResult(ActivityItineraries.this, REQUEST_CHECK_SETTINGS);
    ...
    public class SyncGetLocations extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            // Set data to adapter
            mAdapter.updateList(mItineraryIds, mItineraryNames,mItineraryImages, mItineraryDescription, mItineraryDistancesString);
    ...
    private void getLocationDataFromDatabase(){
        ...
        mItineraryIds.clear();
        mItineraryNames.clear();
        mItineraryDescription.clear();
        mItineraryImages.clear();
        mItineraryLatitudes.clear();
        mItineraryLongitudes.clear();
        mItineraryDistances.clear();
        mItineraryDistancesString.clear();
        ...
        ArrayList<Itineraries> dataLocation = mDBHelper.getObjectItineraries();
        float[] distances = new float[1];

        for(int i = 0; i< dataLocation.size(); i++){
            Itineraries itineraries = dataLocation.get(i);

            // Check distance between locations and user position first
            Location.distanceBetween(Double.valueOf(itineraries.getItineraryStratingLatitude().toString()),
                    Double.valueOf(itineraries.getItineraryStartingLongitude().toString()),
            ...
            if (maxDistancePoi > (distances[0] / 1000.0)) {
                mItineraryIds.add(String.valueOf(itineraries.getItineraryId()));
                mItineraryNames.add(itineraries.getItineraryName());
                mItineraryDescription.add(itineraries.getItineraryDescription());
                mItineraryImages.add(itineraries.getItineraryImage());
                mItineraryLatitudes.add(itineraries.getItineraryStratingLatitude());
                mItineraryLongitudes.add(itineraries.getItineraryStartingLongitude());
                mItineraryMarkers.add(itineraries.getItineraryMarker());


                float paramDistance = (distances[0] / 1000);
                mItineraryDistances.add(paramDistance);

                // For trigger variable mLocationDistancesString
                mItineraryDistancesString.add(String.format("%.2f", paramDistance));
            }
    ...
    private void setupMarker(){
        ...
        for(int i = 0; i< mItineraryLatitudes.size(); i++){

            int marker = getResources().getIdentifier( mItineraryMarkers.get(i), "mipmap", getPackageName());

            Marker locationMarker = mMap.addMarker(new MarkerOptions()
                    .position(new LatLng(Double.valueOf(mItineraryLatitudes.get(i)),Double.valueOf(mItineraryLongitudes.get(i))))
                    .icon(BitmapDescriptorFactory.fromResource(marker))
                    .title(mItineraryNames.get(i)));

            mLocationIdsOnMarkers.put(locationMarker.getId(), mItineraryIds.get(i));

        }

        if(mItineraryLongitudes.size() > 0) {
            showMessage("Sono stati trovati " + mItineraryLongitudes.size());
        }
        else {
    ...
    private void sortDataByDistance()    {
        ...
        while ( flag )
        {
            ...
            for( j = 0;  j < mItineraryDistances.size() - 1;  j++ )
            {
                // change to > for ascending sort, < for descending
                if ( mItineraryDistances.get(j)> mItineraryDistances.get(j+1) )
                {
                    //swap elements
                    tempDistance    = mItineraryDistances.get(j);
                    tempId          = mItineraryIds.get(j);
                    tempName        = mItineraryNames.get(j);
                    tempLatitude    = mItineraryLatitudes.get(j);
                    tempLongitude   = mItineraryLongitudes.get(j);
                    //tempAddress     = mLocationAddresses.get(j);
                    tempImage       = mItineraryImages.get(j);
                    tempMarker      = mItineraryMarkers.get(j);

                    mItineraryDistances.set(j, mItineraryDistances.get(j + 1));

                    // Setting mLocationDistancesString in string format with 2 decimal
                    mItineraryDistancesString.set(j, String.format("%.2f", mItineraryDistances.get(j + 1)));

                    mItineraryIds.set(j, mItineraryIds.get(j+1));
                    mItineraryNames.set(j, mItineraryNames.get(j+1));
                    mItineraryLatitudes.set(j,mItineraryLatitudes.get(j+1));
                    mItineraryLongitudes.set(j,mItineraryLongitudes.get(j+1));
                    //mLocationAddresses.set(j,mLocationAddresses.get(j+1));
                    mItineraryImages.set(j,mItineraryImages.get(j+1));
                    mItineraryMarkers.set(j,mItineraryMarkers.get(j+1));

                    mItineraryDistances.set(j + 1, tempDistance);
                    mItineraryDistancesString.set(j + 1, String.format("%.2f", tempDistance));

                    mItineraryIds.set(j+1,tempId);
                    mItineraryNames.set(j+1,tempName);
                    mItineraryLatitudes.set(j+1,tempLatitude);
                    mItineraryLongitudes.set(j+1,tempLongitude);
                    //mLocationAddresses.set(j+1,tempAddress);
                    mItineraryImages.set(j+1,tempImage);
                    mItineraryMarkers.set(j+1,tempMarker);

                    flag = true;              //shows a swap occurred
                } else {
                    // Setting mLocationDistancesString in string format with 2 decimal
                    mItineraryDistancesString.set(j, String.format("%.2f", mItineraryDistances.get(j)));
                }
            }
        }
    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.menu_activity_itineraries, menu);
        mMenu = menu;
        return true;
    }
